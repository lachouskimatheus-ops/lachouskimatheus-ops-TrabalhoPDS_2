...
O Truco possui integração multiplayer com o frontend por meio de rotas HTTP e WebSocket. A API cria salas, identifica os jogadores e encaminha as ações para o núcleo do jogo.

### Arquivos principais da API

- `SalaTruco.hpp` / `SalaTruco.cpp`: mantém a partida, os jogadores, as equipes e as conexões.
- `TrucoWebSocket.hpp` / `TrucoWebSocket.cpp`: recebe mensagens do frontend e envia o estado atualizado.
- `GerenciadorSalasTruco.hpp` / `GerenciadorSalasTruco.cpp`: cria e localiza salas.
- `TrucoRoutes.hpp` / `TrucoRoutes.cpp`: registra as rotas HTTP relacionadas ao jogo.

### Fluxo da comunicação

```text
Frontend → JSON/WebSocket → TrucoWebSocket → SalaTruco → Truco/Juiz
Truco → Estado atualizado em JSON → Frontend
```

A sala também define a variante da partida e organiza os jogadores em equipes. O WebSocket recebe ações como jogar carta, pedir Truco e responder à aposta, sempre associando a mensagem ao jogador que a enviou.

### Máquina de estados da partida

O fluxo multiplayer é controlado por fases, evitando ações fora do momento correto:
...
- Criar salas para partidas de Truco.
- Permitir escolha entre as variantes Paulista e Mineira.
- Registrar jogadores e equipes.
- Permitir reconexão por token.
- Processar jogadas de carta e pedidos de Truco.
- Processar respostas de aceite, recusa ou aumento da aposta.
- Atualizar placar, turno e estado da mão no frontend.
- Manter ocultas as cartas dos demais jogadores.

## Requisitos não funcionais adicionais

- Utilizar JSON e WebSocket.
- Manter as regras e a arbitragem no backend.
- Separar núcleo, juiz, sala, rotas e comunicação.
- Utilizar polimorfismo para as variantes de Truco.
...
- Manter a instância da partida.
- Registrar jogadores, equipes e conexões.
- Criar o juiz adequado à variante escolhida.
- Controlar entrada, início e reconexão.
...
### Classe TrucoWebSocket

**Responsabilidades:**

- Receber mensagens JSON do frontend.
- Identificar jogador e sala.
- Encaminhar jogadas e apostas ao núcleo.
- Enviar estado, placar e erros.

**Colabora com:**

- `SalaTruco`
- `Truco`
- `GerenciadorSalasTruco`

### Classe GerenciadorSalasTruco
