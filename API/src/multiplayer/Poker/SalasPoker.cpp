...
const ConexaoPoker* SalaPoker::buscarConexaoDoJogador(int idJogador) const {
    for (const ConexaoPoker& registro : conexoes_) {
        if (registro.idJogador == idJogador) return &registro;
    }
...
int SalaPoker::adicionarJogador(crow::websocket::connection* conexao,
...
int SalaPoker::reconectarJogador(crow::websocket::connection* conexao,
                                 const std::string& tokenReconexao) {
    if (conexao == nullptr || tokenReconexao.empty()) return -1;
...
bool SalaPoker::confirmarTroca(int idJogador, const std::vector<int>& indices) {
    return partidaIniciada_ &&
           jogadorEstaConectado(idJogador) &&
           jogo_.confirmarTroca(idJogador, indices);
}
...
std::string SalaPoker::faseParaString(FasePoker fase) {
    switch (fase) {
        case FasePoker::AguardandoJogadores:
            return "AGUARDANDO_JOGADORES";

        case FasePoker::EscolhendoTrocas:
            return "ESCOLHENDO_TROCAS";

        case FasePoker::Resultado:
            return "RESULTADO";
    }
...
std::string SalaPoker::modoParaString(ModoPoker modo) {
    return modo == ModoPoker::ContraComputador
        ? "COMPUTADOR"
        : "MULTIPLAYER";
}
...
json SalaPoker::gerarJson(int idJogadorSolicitante) const {
    ...
    std::string fase = partidaIniciada_
        ? faseParaString(jogo_.fase())
        : "AGUARDANDO_JOGADORES";

    if (partidaIniciada_ &&
        jogo_.fase() == FasePoker::EscolhendoTrocas &&
        idValido &&
        jogo_.jogadorConfirmouTroca(idJogadorSolicitante)) {
    ...
    estado["pode_confirmar_troca"] =
        partidaIniciada_ &&
        idValido &&
        jogo_.podeConfirmarTroca(idJogadorSolicitante);

    estado["pode_iniciar_nova_rodada"] =
        resultado && idJogadorSolicitante == 0;

    if (!partidaIniciada_) {
        estado["mensagem"] = "Aguardando os outros jogadores entrarem.";
    } else if (fase == "ESCOLHENDO_TROCAS") {
        estado["mensagem"] = "Selecione até 3 cartas para trocar.";
    } else if (fase == "AGUARDANDO_OUTROS_JOGADORES") {
        estado["mensagem"] =
            "Troca confirmada. Aguardando os outros jogadores.";
    } else if (jogo_.vencedorRodada() == -1) {
        estado["mensagem"] = "A rodada terminou empatada.";
    } else if (jogo_.vencedorRodada() == idJogadorSolicitante) {
        estado["mensagem"] = "Você venceu a rodada!";
    } else {
        estado["mensagem"] =
            jogo_.nomeJogador(jogo_.vencedorRodada()) +
            " venceu a rodada.";
    }
    ...
    if (partidaIniciada_ && idValido) {
        estado["minha_mao"] = JsonConversor::maoParaJson(
            jogo_.jogador(idJogadorSolicitante).verMao()
        );
    }
    ...
    for (int i = 0; i < jogo_.quantidadeJogadores(); i++) {
        ...
        bool computador =
            jogo_.modo() == ModoPoker::ContraComputador && i == 1;

        bool conectado =
            computador || jogadorEstaConectado(i);
        ...
        jogador["quantidade_cartas"] =
            partidaIniciada_
                ? jogo_.jogador(i).tamanhoMao()
                : 0;

        jogador["confirmou_troca"] =
            partidaIniciada_ &&
            jogo_.jogadorConfirmouTroca(i);

        jogador["quantidade_ultima_troca"] =
            jogo_.quantidadeUltimaTroca()[i];

        if (partidaIniciada_ &&
            (resultado || i == idJogadorSolicitante)) {
            jogador["mao"] = JsonConversor::maoParaJson(
                jogo_.jogador(i).verMao()
            );
